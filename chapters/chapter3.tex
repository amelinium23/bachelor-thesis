...
W aplikacji każdy punkt końcowy posiada swój moduł, dzięki czemu można łatwo go zlokalizować. Zostały one nazwane prefiksami ścieżek do poszczególnych punktów końcowych. Przykładowo, punkt końcowy o nazwie \textit{user}, będzie zdefiniowany w folderze \textit{user}. W każdym pakiecie znajdą się podpakiety, gdzie można znaleźć definicję struktur danych (moduł \textit{model}), bądź dodatkowe funkcję pomocnicze funkcję (moduł \textit{utils}). Wyjątkiem w tej architekturze są dwa moduły: \textit{main} oraz \textit{docs}.
